...
export function CourseMaterialUpload({ courseId, type }: CourseMaterialUploadProps) {
  ...
  const uploadMutation = useMutation({
    mutationFn: async (data: FormData) => {
      const response = await apiRequest("POST", `https://oplayeni.onrender.com/api/courses/${courseId}/materials/upload`, data, { isFormData: true });
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["https://oplayeni.onrender.com/api/courses", courseId, "materials"] });
