...
function CourseCard({ course }: { course: Course }) {
  ...
  const { data: materials } = useQuery<CourseMaterial[]>({
    ...
    queryFn: async () => {
      const response = await apiRequest("GET", `https://oplayeni.onrender.com/api/courses/${course._id}/materials`);
      return response.json();
    }
  });
  ...
  const handleDownload = async (material: CourseMaterial) => {
    try {
      const response = await apiRequest(
        "GET", 
        `https://oplayeni.onrender.com/api/materials/${material._id}/download`,
        undefined,
        { responseType: "blob" }
      );
